...
export default function App() {
    const [targetFid, setTargetFid] = useState('19267');
    const [currentUserProfile, setCurrentUserProfile] = useState(null);

    // Görünüm Modu: 'dashboard' veya 'lists'
    ...
    const [mutuals, setMutuals] = useState([]); // Karşılıklı takipleşenler
    const [recentFollowers, setRecentFollowers] = useState([]); // En son takip edenler
    ...
    const analyzeFollowers = async () => {
        ...
        try {
            ...
            const following = await fetchUsersWithCursor(`https://api.neynar.com/v2/farcaster/following`, targetFid, "Following");

            setLoadingStatus("Scanning Followers list...");
            const followers = await fetchUsersWithCursor(`https://api.neynar.com/v2/farcaster/followers`, targetFid, "Followers");

            // 3. Analiz ve İstatistikler
            ...
            // Neynar API genelde son takip edenleri ilk sayfada verir.
            // İlk 5 kişiyi "Son Takipçiler" olarak alıyoruz.
    ...
    const renderDashboard = () => (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="bg-slate-800 p-6 rounded-xl border border-slate-700">
                ...
                {/* Bar 2: Health Check (Mutuals vs One-Sided) */}
                <div>
                    <div className="flex justify-between text-xs text-slate-400 mb-1">
                    ...
                    <div className="text-[10px] text-slate-500 mt-1">
                        A higher blue bar indicates a strong, engaged community.
                    </div>
                </div>
            </div>

            {/* Son Takipçiler (Newest) */}
                                            ...
                                            (activeTab === 'unfollow' ? notFollowingBack : fans).map(user => (
                                                <div key={user.fid} className="flex items-center justify-between p-3 hover:bg-slate-700/50 rounded-lg transition-colors group border-b border-slate-700/50 last:border-0">
                                                    ...
                                                    <button
                                                        ...
                                                        className={`px-3 py-1.5 rounded text-xs font-bold flex items-center gap-1 transition-colors ${activeTab === 'unfollow'
